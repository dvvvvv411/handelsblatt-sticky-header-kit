...
const AnalyticsPage: React.FC = () => {
  ...
  const statCards = [
    { label: 'Total URLs', value: clickData.length, icon: Link, iconBg: 'bg-gradient-to-br from-blue-500 to-cyan-400', borderColor: 'border-blue-100' },
    { label: 'Total Clicks', value: totalClicks, icon: TrendingUp, iconBg: 'bg-gradient-to-br from-violet-500 to-purple-400', borderColor: 'border-violet-100' },
    { label: 'Avg. Clicks/URL', value: clickData.length > 0 ? Math.round(totalClicks / clickData.length) : 0, icon: BarChart3, iconBg: 'bg-gradient-to-br from-emerald-500 to-teal-400', borderColor: 'border-emerald-100' },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold bg-gradient-to-r from-slate-900 via-indigo-900 to-violet-900 bg-clip-text text-transparent">Click Analytics</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-sm">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            <div className="w-16 h-16 bg-gradient-to-br from-violet-50 to-indigo-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 bg-gradient-to-r from-slate-50/80 to-indigo-50/30">
                  <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Short Code</th>
                  <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Article</th>
                  <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Clicks</th>
                  <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Original URL</th>
              ...
              <tbody className="divide-y divide-slate-100/80">
                {clickData.map((item) => (
                  <tr key={item.id} className="hover:bg-gradient-to-r hover:from-indigo-50/30 hover:to-transparent transition-colors">
                    <td className="py-4 px-4">
                      <button
                        onClick={() => copyShortUrl(item.short_code)}
                        className="font-mono text-sm bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-2.5 py-1 rounded-lg transition-colors flex items-center gap-1.5"
                      >
                        {item.short_code}
