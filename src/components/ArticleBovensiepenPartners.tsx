...
interface ArticleBovensiepenPartnersProps {
  articleId?: string;
  bovensiepenPartnersUrl?: string;
}

const ArticleBovensiepenPartners: React.FC<ArticleBovensiepenPartnersProps> = ({ 
  articleId, 
  bovensiepenPartnersUrl = 'https://bovensiepen-partner.com?ref=handelsblatt' 
}) => {
  const [shortUrl, setShortUrl] = useState<string>(bovensiepenPartnersUrl);
  ...
  useEffect(() => {
    const generateShortUrl = async () => {
      if (articleId) {
        ...
        try {
          const generated = await createShortUrl(bovensiepenPartnersUrl, articleId);
          if (generated) {
    ...
    generateShortUrl();
  }, [articleId, bovensiepenPartnersUrl]);
  ...
  return (
    <div className="relative my-6 md:my-8">
      ...
      {/* Bovensiepen & Partner Advertisement Card - Mobile Responsive */}
      <div 
...
export default ArticleBovensiepenPartners;